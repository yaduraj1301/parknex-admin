...
console.log('Sample slots generated:', Object.keys(slots).length);
        return slots;
    }
    ...
    simulateRealTimeUpdates() {
        // Only update status of existing slots, don't create/remove slots
        ...
        for (let i = 0; i < Math.floor(Math.random() * 2) + 2; i++) {
            ...
            // Only update if the slot exists
            if (this.slots[randomSlotId]) {
        ...
        // Re-render the current level to show changes
        this.renderParkingSlots();

        console.log('Real-time update completed - status changes only');
    }
